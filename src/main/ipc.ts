...
import type { Snippet, Collection, Tag, CreateSnippetInput, UpdateSnippetInput, CreateCollectionInput } from '../shared/types';
...
export function registerIpcHandlers(): void {
  ...
  ipcMain.handle('snippets:update', (_event, input: UpdateSnippetInput): Snippet => {
    ...
    return db.prepare('SELECT * FROM snippets WHERE id = ?').get(input.id) as Snippet;
  });
